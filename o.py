"""
    ...
    如果frame rate不一樣, 也會把多的幀數去掉, 變成一樣的frame rate
...
# 設定原影片與要比較相似度的影片
input_video = 'pose1.mp4'
compare_video = 'pose1_20241107131722.mp4'
...
class skeleton_detection_similarity:
    def __init__(self, video1, video2):
        ...
        # 設定目標幀率為兩者的最小值，以避免幀重複
        self.target_fps = min(fps1, fps2)
    ...
    def run(self):
        ...
        with self.mp_pose.Pose(
            ...
            while True:
                # 目標幀的時間點
                self.frame_time += self.frame_interval

                # 讀取 video1 的幀直到其時間 >= frame_time
                frame1 = None
                while self.time1 < self.frame_time:
                    ret1, temp_frame1 = self.cap1.read()
                    if not ret1:
                        break
                    frame1 = temp_frame1
                    self.time1 += 1 / self.cap1.get(cv2.CAP_PROP_FPS)

                # 讀取 video2 的幀直到其時間 >= frame_time
                frame2 = None
                while self.time2 < self.frame_time:
                    ret2, temp_frame2 = self.cap2.read()
                    if not ret2:
                        break
                    frame2 = temp_frame2
                    self.time2 += 1 / self.cap2.get(cv2.CAP_PROP_FPS)

                # 檢查是否有幀可讀
                if frame1 is None or frame2 is None:
                    print("Cannot receive frame or end of one video reached")
                    break
